...
for i in range(0,10):
    ...
    axes[i].plot(time+1.,tr.data,color='C1',label='AxiSEM',zorder=2)
    ...
    tr.filter('bandpass',freqmin=freqmin,freqmax=freqmax,corners=corners,zerophase=zerophase)
    time = np.linspace(0,tr.stats.npts*tr.stats.delta,tr.stats.npts)
    axes[i].plot(time,tr.data,color='C2',label='OpenSWPC',zorder=3)
    ...
    axes[i].plot(time,tr.data,color='C3',label='SPECFEM3D',zorder=1)

    #Axis Params-----------------------------------------------------------
    #axes[i].set_xlim([0,100])
    axes[i].set_xlim([0,30])
    axes[i].set_ylabel('vel. (m/s)')
    if i==4 : axes[i].legend(loc='upper right')
...
plt.savefig('AxiSEM_vs_OpenSWPC_vs_Specfem_waveforms_D01-10.pdf',transparent=True)
