...
for i in range(0,5):
    ...
    #AxiSEM-----------------------------------------------------------------
    st = obspy.read("./AxiSEM_SEIS_BM_v2/sac_seismograms/{}km_Vz.sac".format(i+5)) #sac seismograms are in velocity
    tr = st[0]
    ...
    #OpenSWPC--------------------------------------------------------------
    st = obspy.read("./Benchmark_KO_20191030/Vz_m:s/{}km_Vz.sac".format(i+5))
    tr = st[0]
    tr.filter('bandpass',freqmin=freqmin,freqmax=freqmax,corners=corners,zerophase=zerophase)
    time = np.linspace(0,tr.stats.npts*tr.stats.delta,tr.stats.npts)
    ...
    #Specfem---------------------------------------------------------------
    #st = obspy.read("./SEM_RESULT_TOMOBM_KUPP/D{:02d}.KO.FXZ.semv.sac".format(i+5))
    st = obspy.read("./SEM_RESULT_TOMO_FAR/D{:02d}.KO.FXZ.semv.sac".format(i+5))
    tr = st[0]
    tr.filter('bandpass',freqmin=freqmin,freqmax=freqmax,corners=corners,zerophase=zerophase)
    ...
    axes[i].set_title('dist {} km'.format(i+5))
...
plt.savefig('AxiSEM_vs_OpenSWPC_vs_Specfem_waveforms_far.pdf',transparent=True)
